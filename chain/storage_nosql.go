...
const (
	...
	NoSQLBlockDelimiter byte = 0x0
	NoSQLByteDelimiter  byte = '/'
)
...
type KeyWriteQueryValueMeta struct {
	Key                 string               `serialize:"true" json:"key"`
	WriteQueryValueMeta *WriteQueryValueMeta `serialize:"true" json:"writeQueryValueMeta"`
}
type WriteQueryValueMeta struct {
	ContractTxId   string `serialize:"true" json:"contract_tx_id"`
	CollectionPath string `serialize:"true" json:"collection_paht"`
...
//	func PutKey(db database.KeyValueWriter, key common.Hash, vmeta *WriteQueryValueMeta) error {
//		// [keyPrefix] + [delimiter] + [key]
//		k := ValueKey(key)
//		rvmeta, err := Marshal(vmeta)
//		if err != nil {
...
//		return db.Put(k, rvmeta)
//	}
func WriteTestQuery(db database.KeyValueReaderWriter, contractTxId string, collectionPath string, query string) error {

	// k := PrefixBalanceKey(address)
	// copy(k[2:], []byte(contractTxId))
	// b := make([]byte, 8)
	// binary.BigEndian.PutUint64(b, bal)
	// 	binary.BigEndian.PutUint64(b, bal)
	// 	return db.Put(k, b)
	k := make([]byte, 2+len(contractTxId))

	k[0] = NoSQLBlockDelimiter
	k[1] = NoSQLByteDelimiter
	copy(k[2:], contractTxId)
