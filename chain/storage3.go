...
const (
	...
	linkedTxLRUSize3 = 512

	ByteDelimiter3 byte = '/'
)

var (
	lastAccepted3  = []byte("last_accepted")
	linkedTxCache3 = &cache.LRU{Size: linkedTxLRUSize}
)

// [blockPrefix] + [delimiter] + [blockID]
func PrefixBlockKey3(blockID ids.ID) (k []byte) {
...
func PrefixTxKey3(txID ids.ID) (k []byte) {
...
func PrefixTxValueKey3(txID ids.ID) (k []byte) {
	k = make([]byte, 2+len(txID))
	k[0] = txValuePrefix
	k[1] = ByteDelimiter
	copy(k[2:], txID[:])
...
func ValueKey3(key common.Hash) (k []byte) {
...
func PrefixBalanceKey3(address common.Address) (k []byte) {
...
var ErrInvalidKeyFormat3 = errors.New("invalid key format")

func GetValueMeta3(db database.KeyValueReader, key common.Hash) (*ValueMeta, bool, error) {
	// [keyPrefix] + [delimiter] + [key]
	k := ValueKey(key)
	rvmeta, err := db.Get(k)
...
func GetValue3(db database.KeyValueReader, key common.Hash) ([]byte, bool, error) {
	// [keyPrefix] + [delimiter] + [key]
	k := ValueKey(key)
	rvmeta, err := db.Get(k)
	if errors.Is(err, database.ErrNotFound) {
...
// written to disk.
func linkValues3(db database.KeyValueWriter, block *StatelessBlock) ([]*Transaction, error) {
	g := block.vm.Genesis()
...
// in [block].
func restoreValues3(db database.KeyValueReader, block *StatefulBlock) error {
...
func SetLastAccepted3(db database.KeyValueWriter, block *StatelessBlock) error {
	bid := block.ID()
	if err := db.Put(lastAccepted, bid[:]); err != nil {
		return err
	}
	ogTxs, err := linkValues3(db, block)
	if err != nil {
		return err
	}
	...
	if err := db.Put(PrefixBlockKey3(bid), sbytes); err != nil {
		return err
	}
...
func HasLastAccepted3(db database.Database) (bool, error) {
	return db.Has(lastAccepted)
}

func GetLastAccepted3(db database.KeyValueReader) (ids.ID, error) {
...
func GetBlock3(db database.KeyValueReader, bid ids.ID) (*StatefulBlock, error) {
	b, err := db.Get(PrefixBlockKey3(bid))
	if err != nil {
		return nil, err
	}
	...
	if err := restoreValues3(db, blk); err != nil {
		return nil, err
	}
	return blk, nil
}
...
// DB
func HasKey3(db database.KeyValueReader, key common.Hash) (bool, error) {
	// [keyPrefix] + [delimiter] + [key]
	k := ValueKey(key)
	return db.Has(k)
}

type ValueMeta3 struct {
	Size    uint64 `serialize:"true" json:"size"`
	TxID    ids.ID `serialize:"true" json:"txId"`
	Created uint64 `serialize:"true" json:"created"`
}

func PutKey3(db database.KeyValueWriter, key common.Hash, vmeta *ValueMeta) error {
	// [keyPrefix] + [delimiter] + [key]
	k := ValueKey(key)
...
func SetTransaction3(db database.KeyValueWriter, tx *Transaction3) error {
	k := PrefixTxKey(tx.ID())
	return db.Put(k, nil)
}
